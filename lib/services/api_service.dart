  ...
  ApiService() {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      onRequest: (options, handler) async {
        // JWT 토큰 자동 헤더 추가 (POST /profiles 요청은 일시적으로 건너뛰기)
        final isProfilePost = options.method == 'POST' && options.path.contains('/profiles');
        
        if (isProfilePost) {
          print('[API][AUTH] 프로필 POST 요청 - 인증 건너뛰기 (임시)');
        } else {
          try {
