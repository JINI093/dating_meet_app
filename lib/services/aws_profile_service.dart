...
class AWSProfileService {
  ...
  Future<void> initialize() async {
    try {
      ...
      Logger.error('❌ AWSProfileService 초기화 실패',
          error: e, name: 'AWSProfileService');
      rethrow;
    }
  }
  ...
  Future<ProfileModel?> createProfile({
    ...
    try {
      ...
      final uploadedImageUrls =
          await _uploadProfileImages(userId, profileImages);
      ...
      final profileId =
          '${now.millisecondsSinceEpoch}-${_uuid.v4().substring(0, 8)}';
      final profileData = {
      ...
      Logger.log(
          '프로필 데이터: userId=${profileData['userId']}, name=${profileData['name']}, images=${uploadedImageUrls.length}장',
          name: 'AWSProfileService');

      // 4. API 호출하여 프로필 저장 (임시 사용자는 건너뛰기)
      if (userId.startsWith('temp_user_')) {
        Logger.log('임시 사용자이므로 GraphQL API 호출 건너뛰고 바로 로컬 프로필 생성',
            name: 'AWSProfileService');
        ...
        Logger.log('임시 사용자용 로컬 프로필 생성 완료: ${localProfile.id}',
            name: 'AWSProfileService');
        return localProfile;
      }
      ...
      try {
        ...
        Logger.log('REST API 요청 데이터: ${profileData.keys.join(', ')}',
            name: 'AWSProfileService');
        Logger.log(
            '주요 필드 값 확인: name=${profileData['name']}, age=${profileData['age']}, userId=${profileData['userId']}',
            name: 'AWSProfileService');
        Logger.log('전체 profileData: ${json.encode(profileData)}',
            name: 'AWSProfileService');

        final response = await apiService.post('/profiles', data: profileData);

        Logger.log(
            'REST API 응답: statusCode=${response.statusCode}, data=${response.data != null ? 'exists' : 'null'}',
            name: 'AWSProfileService');
        Logger.log('REST API 응답 내용: ${response.data}',
            name: 'AWSProfileService');
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          if (response.data != null &&
            ...
            Logger.error('백엔드에서 에러 반환: ${response.data['errorMessage']}',
                name: 'AWSProfileService');
          ...
          if (response.data != null && response.data is Map<String, dynamic>) {
            try {
              ...
              if (responseMap.containsKey('body') &&
                  responseMap['body'] is String) {
                final bodyString = responseMap['body'] as String;
                final bodyData =
                    json.decode(bodyString) as Map<String, dynamic>;
              ...
              Logger.log('응답 데이터 구조가 예상과 다르지만 API는 성공. 원본 데이터로 프로필 생성',
                  name: 'AWSProfileService');
            ...
            Logger.log('응답 데이터가 올바른 형식이 아니지만 API는 성공. 원본 데이터로 프로필 생성',
                name: 'AWSProfileService');
          ...
          Logger.error(
              'REST API 응답 상태 코드: ${response.statusCode}, 응답: ${response.data}',
              name: 'AWSProfileService');
        ...
        if (e.toString().contains('403')) {
          Logger.error('403 인증 오류: AWS API Gateway 또는 Lambda 권한 설정을 확인하세요',
              name: 'AWSProfileService');
  ...
  Future<ProfileModel?> updateProfile({
    ...
    try {
      ...
      List<String> updatedImageUrls =
          existingImageUrls ?? existingProfile.profileImages;

      if (newProfileImages != null && newProfileImages.isNotEmpty) {
        final newImageUrls =
            await _uploadProfileImages(existingProfile.id, newProfileImages);
      ...
      if (additionalData != null) {
        ...
        for (final entry in additionalData.entries) {
          if (validFields.contains(entry.key)) {
            ...
            Logger.log('GraphQL 스키마에 없는 필드 무시: ${entry.key}',
                name: 'AWSProfileService');
          }
        }
      }
      ...
      if (response.errors.isNotEmpty) {
        ...
        Logger.log('조건부 업데이트 실패, 재시도: ${response.errors.first.message}',
            name: 'AWSProfileService');
        ...
        if (retryResponse.errors.isNotEmpty) {
          ...
          Logger.log(
              '업데이트 재시도 실패, 프로필 생성 시도: ${retryResponse.errors.first.message}',
              name: 'AWSProfileService');
          ...
          if (createResponse.errors.isNotEmpty) {
            throw Exception(
                '프로필 생성 실패: ${createResponse.errors.first.message}');
          }

          if (createResponse.data != null) {
            try {
              Logger.log('프로필 생성 응답: ${createResponse.data}',
                  name: 'AWSProfileService');
              Logger.log('응답 타입: ${createResponse.data.runtimeType}',
                  name: 'AWSProfileService');

              dynamic profileData;
              if (createResponse.data is String) {
                final parsedData = json.decode(createResponse.data as String)
                ...
                throw Exception(
                    '예상치 못한 응답 형식: ${createResponse.data.runtimeType}');
              }
  ...
  Future<ProfileModel?> getProfile(String userId,
    ...
    try {
      Logger.log('프로필 조회 시작: $userId (forceRefresh: $forceRefresh)',
          name: 'AWSProfileService');
      ...
      if (!forceRefresh) {
        ...
        if (cachedProfile != null) {
          Logger.log('캐시에서 프로필 로드 성공: ${cachedProfile.name}',
              name: 'AWSProfileService');
      ...
      if (_ongoingRequests.contains(userId)) {
        Logger.log('이미 진행 중인 요청이 있음, 잠시 대기: $userId',
            name: 'AWSProfileService');
      ...
      try {
        ...
        if (dynamoProfile != null) {
          Logger.log('DynamoDB에서 프로필 로드 성공: ${dynamoProfile.name}',
              name: 'AWSProfileService');
    ...
    try {
      ...
      if (response.data != null) {
        try {
          ...
          Logger.error('GraphQL 응답 파싱 오류',
              error: parseError, name: 'AWSProfileService');
      ...
      Logger.error('GraphQL 프로필 조회 실패, DynamoDB 직접 스캔으로 재시도: $e',
          name: 'AWSProfileService');
      ...
      try {
        ...
        if (directProfile != null) {
          Logger.log('✅ DynamoDB 직접 스캔으로 프로필 조회 성공: ${directProfile.name}',
              name: 'AWSProfileService');
          return directProfile;
        }
      } catch (directError) {
        Logger.error('DynamoDB 직접 스캔도 실패: $directError',
            name: 'AWSProfileService');
      }
  ...
  Future<List<ProfileModel>> getDiscoverProfiles({
    ...
    try {
      // 캐시 키 생성
      final cacheKey =
          '${currentUserId}_${gender ?? 'all'}_${minAge ?? 0}_${maxAge ?? 100}_${location ?? 'all'}_$limit';

      // 디버깅 로그
      Logger.log('=== getDiscoverProfiles 디버깅 시작 ===',
          name: 'AWSProfileService');
      ...
      Logger.log(
          '   필터링 조건: minAge=$minAge, maxAge=$maxAge, location=$location, limit=$limit',
          name: 'AWSProfileService');
      ...
      if (cachedProfiles != null) {
        Logger.log('✅ 캐시에서 디스커버 프로필 로드: ${cachedProfiles.length}개',
            name: 'AWSProfileService');
      ...
      try {
        ...
        Logger.log('📝 GraphQL 필터 조건: ${json.encode(filter)}',
            name: 'AWSProfileService');
        ...
        if (response.errors.isNotEmpty) {
          ...
          for (int i = 0; i < response.errors.length; i++) {
            ...
            Logger.error('     메시지: ${error.message}',
                name: 'AWSProfileService');
            Logger.error('     위치: ${error.locations}',
                name: 'AWSProfileService');
            Logger.error('     경로: ${error.path}', name: 'AWSProfileService');
            Logger.error('     확장: ${error.extensions}',
                name: 'AWSProfileService');
          }
          throw Exception(
              'GraphQL 프로필 목록 조회 실패: ${response.errors.first.message}');
        }
        ...
        if (response.data != null) {
          ...
          Logger.log('   타입: ${response.data.runtimeType}',
              name: 'AWSProfileService');
          ...
          try {
            final data = _parseGraphQLResponse(response.data!);
            Logger.log('🔄 파싱된 데이터 구조: ${data.keys.toList()}',
                name: 'AWSProfileService');

            final items = data['listProfiles']?['items'] as List?;
            Logger.log('📊 GraphQL 조회 결과: ${items?.length ?? 0}개 프로필',
                name: 'AWSProfileService');
            ...
            if (items != null && items.isNotEmpty) {
              ...
              for (int i = 0; i < items.length && i < 5; i++) {
                ...
                Logger.log('     원본 데이터: ${item.toString()}',
                    name: 'AWSProfileService');
                Logger.log(
                    '     이름: ${item['name']}, 성별: ${item['gender']}, 나이: ${item['age']}, ID: ${item['id']}',
                    name: 'AWSProfileService');

                // DynamoDB 형식인지 확인
                if (_isDynamoDBFormat(item)) {
                  Logger.log('     ⚠️  DynamoDB 형식 데이터 발견 - 변환 필요',
                      name: 'AWSProfileService');
                  final converted = _convertDynamoDBToJson(item);
                  Logger.log('     🔄 변환된 데이터: ${converted.toString()}',
                      name: 'AWSProfileService');
                }
              }
              ...
              try {
                final profiles = items.map((item) {
                  ...
                  Logger.log(
                      '📝 프로필 생성 데이터: name=${profileData['name']}, gender=${profileData['gender']}',
                      name: 'AWSProfileService');
                  ...
                  if (!shouldInclude) {
                    Logger.log(
                        '❌ 자신의 프로필 제외: ${profile.name} (ID: ${profile.id})',
                        name: 'AWSProfileService');
                  } else {
                    Logger.log(
                        '✅ 포함할 프로필: ${profile.name} (${profile.gender}) - ${profile.age}세',
                        name: 'AWSProfileService');
                  }
                  return shouldInclude;
                }).toList();

                Logger.log('🎯 최종 필터링 결과: ${profiles.length}개 프로필',
                    name: 'AWSProfileService');
                if (profiles.isNotEmpty) {
                  Logger.log('✅ GraphQL을 통한 프로필 조회 성공',
                      name: 'AWSProfileService');
                  return profiles;
                } else {
                ...
                Logger.error('❌ 프로필 파싱 에러:',
                    error: profileParseError, name: 'AWSProfileService');
                Logger.log('프로필 파싱 실패 - REST API 폴백 시도',
                    name: 'AWSProfileService');
              }
            } else {
              Logger.log('⚠️  GraphQL 응답에 프로필 아이템이 없음',
                  name: 'AWSProfileService');
            }
          } catch (parseError) {
            Logger.error('❌ GraphQL 응답 파싱 에러:',
                error: parseError, name: 'AWSProfileService');
            Logger.log('GraphQL 응답 파싱 실패 - REST API 폴백 시도',
                name: 'AWSProfileService');
          }
        } else {
        ...
        Logger.log('📡 GraphQL 조회 실패 또는 결과 없음 - REST API 폴백 시도',
            name: 'AWSProfileService');
      } catch (e) {
        Logger.error('❌ GraphQL 매칭 프로필 조회 오류:',
            error: e, name: 'AWSProfileService');
      ...
      try {
        ...
        Logger.log('📝 REST API 요청 파라미터: ${json.encode(queryParams)}',
            name: 'AWSProfileService');

        // 프로필 API는 별도 API Gateway 사용
        final profileApiService = Dio(BaseOptions(
          baseUrl:
              'https://wkj6fdmoyf.execute-api.ap-northeast-2.amazonaws.com/dev',
        ...
        try {
          ...
          if (session.isSignedIn &&
              session.userPoolTokensResult.value != null) {
            final idToken = session.userPoolTokensResult.value!.idToken.raw;
            if (idToken.isNotEmpty) {
              profileApiService.options.headers['Authorization'] =
                  'Bearer $idToken';
            }
          }
        } catch (e) {
        ...
        final response = await profileApiService.get('/profiles/discover',
            queryParameters: queryParams);

        Logger.log('📡 REST API 응답:', name: 'AWSProfileService');
        Logger.log('   상태 코드: ${response.statusCode}',
            name: 'AWSProfileService');
        Logger.log('   응답 타입: ${response.data.runtimeType}',
            name: 'AWSProfileService');
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            if (data is Map &&
              ...
              Logger.log('🔄 Lambda body 파싱: ${bodyData.toString()}',
                  name: 'AWSProfileService');

              if (bodyData['success'] == true && bodyData['data'] is List) {
                profilesData = bodyData['data'] as List;
                Logger.log(
                    '📋 Lambda body.data에서 데이터 추출: ${profilesData.length}개',
                    name: 'AWSProfileService');
              }
            } else if (data is List) {
              profilesData = data;
              Logger.log('📋 직접 리스트 데이터: ${profilesData.length}개',
                  name: 'AWSProfileService');
            }

            if (profilesData != null && profilesData.isNotEmpty) {
              Logger.log('🔄 REST API 프로필 데이터 변환 시작',
                  name: 'AWSProfileService');

              final profiles = profilesData.map((item) {
                final itemMap = item as Map<String, dynamic>;
                Logger.log('📝 REST API 프로필 아이템: ${itemMap.toString()}',
                    name: 'AWSProfileService');
                ...
                Logger.log(
                    '🔄 변환된 프로필 데이터: name=${profileData['name']}, gender=${profileData['gender']}',
                    name: 'AWSProfileService');
                ...
                if (!shouldInclude) {
                  Logger.log(
                      '❌ REST API - 자신의 프로필 제외: ${profile.name} (${profile.id})',
                      name: 'AWSProfileService');
                } else {
                  Logger.log(
                      '✅ REST API - 포함할 프로필: ${profile.name} (${profile.gender}) - ${profile.age}세',
                      name: 'AWSProfileService');
                }
                return shouldInclude;
              }).toList();

              Logger.log('🎯 REST API 최종 결과: ${profiles.length}개 프로필',
                  name: 'AWSProfileService');

              if (profiles.isNotEmpty) {
                Logger.log('✅ REST API를 통한 매칭 프로필 조회 성공',
                    name: 'AWSProfileService');
                // _cacheDiscoverProfiles(cacheKey, profiles); // TODO: 캐시 키 스코프 문제로 임시 주석
                return profiles;
              } else {
                Logger.log('⚠️  REST API 필터링 후 프로필이 없음',
                    name: 'AWSProfileService');
              }
            } else {
              Logger.log('⚠️  REST API 응답에 프로필 데이터가 없음',
                  name: 'AWSProfileService');
            }
          } catch (restParseError) {
            Logger.error('❌ REST API 응답 파싱 에러:',
                error: restParseError, name: 'AWSProfileService');
            Logger.log('REST API 파싱 실패 상세: ${restParseError.toString()}',
                name: 'AWSProfileService');
          }
        } else if (response.statusCode == 401 || response.statusCode == 403) {
          Logger.error('🔐 REST API 인증 오류: ${response.statusCode}',
              name: 'AWSProfileService');
        } else if (response.statusCode != null && response.statusCode! >= 500) {
          Logger.error('🚨 REST API 서버 오류: ${response.statusCode}',
              name: 'AWSProfileService');
        } else {
          Logger.error('❌ REST API 응답 오류: ${response.statusCode}',
              name: 'AWSProfileService');
        }
      } catch (restError) {
        Logger.error('❌ REST API 매칭 프로필 조회 실패:',
            error: restError, name: 'AWSProfileService');
        Logger.log('REST API 오류 상세: ${restError.toString()}',
            name: 'AWSProfileService');
      ...
      Logger.log('  샘플 데이터 생성 정책: ${shouldGenerateSampleData ? "활성화" : "비활성화"}',
          name: 'AWSProfileService');
      Logger.log('  조건: 임시 사용자, 테스트 사용자, 개발 사용자인 경우만 활성화',
          name: 'AWSProfileService');
      ...
      if (shouldGenerateSampleData) {
        ...
        Logger.log('✅ 샘플 데이터 생성 완료: ${sampleProfiles.length}개',
            name: 'AWSProfileService');
        return sampleProfiles;
      } else {
        Logger.log('📋 실제 데이터 우선 정책으로 빈 리스트 반환', name: 'AWSProfileService');
        Logger.log('  이유: AWS API에서 실제 데이터를 가져오지 못하는 정확한 원인을 파악하기 위함',
            name: 'AWSProfileService');
        Logger.log('  권장사항: 위의 에러 로그를 확인하여 AWS 설정 문제를 해결하세요',
            name: 'AWSProfileService');
      }

      return []; // 빈 리스트 반환으로 문제 상황을 명확히 표시
    } catch (e) {
      Logger.error('❌ getDiscoverProfiles 오류:',
          error: e, name: 'AWSProfileService');
      return [];
    } finally {
  ...
  Future<List<ProfileModel>> getVipProfiles({
    ...
    try {
      // 캐시 키 생성
      final cacheKey =
          'vip_${currentUserId}_${gender ?? 'all'}_${vipGrade ?? 'all'}_${minAge ?? 0}_${maxAge ?? 100}_${location ?? 'all'}_$limit';
      ...
      final cachedProfiles = _getCachedDiscoverProfiles(cacheKey);
      if (cachedProfiles != null) {
        Logger.log('✅ 캐시에서 VIP 프로필 로드: ${cachedProfiles.length}개',
            name: 'AWSProfileService');
        return cachedProfiles;
      }
      ...
      try {
        ...
        Logger.log('📝 VIP GraphQL 필터 조건: ${json.encode(filter)}',
            name: 'AWSProfileService');
        ...
        if (response.errors.isNotEmpty) {
          Logger.error('VIP GraphQL 오류: ${response.errors}',
              name: 'AWSProfileService');
          return [];
        }
        ...
        Logger.log('📊 VIP API 응답: ${items.length}개 프로필',
            name: 'AWSProfileService');
        ...
        for (final item in items) {
          try {
            ...
            if (item['userId'] == currentUserId) {
              Logger.log('본인 프로필 제외: ${item['name']}',
                  name: 'AWSProfileService');
              continue;
            }
            ...
            Logger.log(
                '✅ VIP 프로필 파싱 성공: ${profile.name} (${profile.age}세, ${profile.gender})',
                name: 'AWSProfileService');
          } catch (e) {
            Logger.error('VIP 프로필 파싱 실패: $e', name: 'AWSProfileService');
            Logger.error('문제가 된 데이터: ${json.encode(item)}',
                name: 'AWSProfileService');
          }
        }
        ...
        Logger.log('🎯 최종 VIP 프로필 리스트: ${profiles.length}개',
            name: 'AWSProfileService');
        profiles.asMap().forEach((index, profile) {
          Logger.log(
              '   [$index] ${profile.name} (${profile.age}세, ${profile.gender}, VIP: ${profile.isVip})',
              name: 'AWSProfileService');
        });

        return profiles;
      } catch (e) {
        Logger.error('❌ 내부 getVipProfiles 오류:',
            error: e, name: 'AWSProfileService');
        return [];
      } finally {
      ...
      Logger.error('❌ getVipProfiles 외부 오류:',
          error: e, name: 'AWSProfileService');
      return [];
    }
  }
  ...
  Future<List<String>> _uploadProfileImages(
      String userId, List<File> images) async {
    final uploadedUrls = <String>[];

    try {
      Logger.log('🔄 이미지 S3 업로드 시작: ${images.length}장',
          name: 'AWSProfileService');
      ...
      for (int i = 0; i < images.length; i++) {
        ...
        Logger.log('📸 이미지 ${i + 1}/${images.length} 처리: ${image.path}',
            name: 'AWSProfileService');

        try {
          // 1. 이미지 압축
          final compressedBytes = await _compressImage(image);
          Logger.log('이미지 압축 완료: ${compressedBytes.length} bytes',
              name: 'AWSProfileService');
          ...
          final uploadResult = await Amplify.Storage.uploadData(
            ...
            onTimeout: () {
              throw TimeoutException(
                  'S3 업로드 타임아웃 (30초)', const Duration(seconds: 30));
            },
          );

          Logger.log('S3 업로드 성공: ${uploadResult.uploadedItem.path}',
              name: 'AWSProfileService');

          // 4. 공개 URL 생성 (guest 레벨은 공개 접근 가능)
          final publicUrl =
              'https://meet-project.s3.ap-northeast-2.amazonaws.com/$s3Key';
          uploadedUrls.add(publicUrl);

          Logger.log('✅ 이미지 ${i + 1}/${images.length} 업로드 완료: $publicUrl',
              name: 'AWSProfileService');
        } catch (e) {
          Logger.error('이미지 ${i + 1}/${images.length} 업로드 실패: $e',
              name: 'AWSProfileService');

          // 실패한 이미지는 placeholder 이미지로 대체
          final placeholderUrl =
              'https://picsum.photos/seed/${_uuid.v4()}/400/600';
          uploadedUrls.add(placeholderUrl);
          Logger.log('S3 업로드 실패로 placeholder 이미지 사용: $placeholderUrl',
              name: 'AWSProfileService');
        }
      }

      Logger.log('🎉 이미진 업로드 완료: ${uploadedUrls.length}장',
          name: 'AWSProfileService');
      return uploadedUrls;
    } catch (e) {
      ...
      Logger.log('로컬 경로로 대체 완료: ${uploadedUrls.length}장',
          name: 'AWSProfileService');
      return uploadedUrls;
    }
  }
  ...
  Future<ProfileModel?> _getProfileFromLocal(String userId) async {
    try {
      ...
      if (profileJson != null && profileJson.isNotEmpty) {
        ...
        Logger.log('📄 로컬 저장소 원본 데이터: ${profileData.toString()}',
            name: 'AWSProfileService');
        ...
        Logger.log('🔄 로컬 저장소 변환된 데이터: ${convertedData.toString()}',
            name: 'AWSProfileService');
        ...
        Logger.log('✅ 로컬 저장소에서 프로필 로드 성공: ${safeProfileData['name']}',
            name: 'AWSProfileService');
  ...
  Map<String, dynamic> _parseGraphQLResponse(dynamic response) {
    try {
      ...
      Logger.log('GraphQL 응답 타입 확인: ${response.runtimeType}',
          name: 'AWSProfileService');
  ...
  Future<void> updateOnlineStatus(String profileId, bool isOnline) async {
    try {
      ...
      Logger.log('온라인 상태 업데이트: $profileId -> ${isOnline ? "온라인" : "오프라인"}',
          name: 'AWSProfileService');
  ...
  Future<ProfileModel?> _getProfileFromDynamoDBInternal(String userId) async {
    try {
      ...
      try {
        final directProfile = await _getProfileByUserIdDirect(userId);
        if (directProfile != null) {
          Logger.log('✅ 직접 DynamoDB에서 프로필 로드 성공: ${directProfile.name}',
              name: 'AWSProfileService');
          return directProfile;
        }
        Logger.log('직접 DynamoDB 스캔에서 프로필을 찾지 못함: $userId',
            name: 'AWSProfileService');
      } catch (directError) {
        Logger.log('직접 DynamoDB 조회 실패, GraphQL로 재시도: $directError',
            name: 'AWSProfileService');
      }
      ...
      try {
        ...
        if (graphqlProfile != null) {
          Logger.log('✅ GraphQL에서 프로필 로드 성공: ${graphqlProfile.name}',
              name: 'AWSProfileService');
      ...
      Logger.log('⚠️ DynamoDB 스캔과 GraphQL 모두 실패 - REST API 호출 건너뛰고 로컬 저장소 확인',
          name: 'AWSProfileService');

      Logger.log('❌ DynamoDB 스캔과 GraphQL 모두 실패, REST API 시도: $userId',
          name: 'AWSProfileService');
      ...
      try {
        ...
        Logger.log('REST API 응답 상태: ${response.statusCode}',
            name: 'AWSProfileService');
        Logger.log('REST API 응답 데이터: ${response.data}',
            name: 'AWSProfileService');
        ...
        if (response.statusCode == 200) {
          ...
          if (responseData != null &&
            ...
            Logger.log('REST API 응답 형식 오류: $responseData',
                name: 'AWSProfileService');
          }
        }
      } catch (restError) {
        Logger.error('REST API 프로필 조회도 실패: $restError',
            name: 'AWSProfileService');
      }
  ...
  Future<ProfileModel?> _getProfileByUserIdDirect(String userId) async {
    try {
      Logger.log('🔍 DynamoDB GetItem으로 프로필 조회 시작: $userId',
          name: 'AWSProfileService');
      ...
      if (getResponse.data != null && !getResponse.hasErrors) {
        ...
        if (profile != null) {
          Logger.log('✅ GetItem으로 프로필 발견: ${profile['name']}',
              name: 'AWSProfileService');
      ...
      Logger.log('  hasErrors: ${response.hasErrors}',
          name: 'AWSProfileService');
      Logger.log('  errors: ${response.errors}', name: 'AWSProfileService');
      Logger.log('  data != null: ${response.data != null}',
          name: 'AWSProfileService');

      if (response.errors.isNotEmpty) {
        Logger.error(
            'GraphQL 스캔 에러: ${response.errors.map((e) => e.message).join(", ")}',
            name: 'AWSProfileService');
        return null;
      }
      ...
      if (response.data != null) {
        ...
        try {
          ...
          Logger.log('전체 프로필 수: ${items?.length ?? 0}',
              name: 'AWSProfileService');

          if (items != null && items.isNotEmpty) {
            Logger.log('🔍 검색 중인 userId: $userId', name: 'AWSProfileService');
            ...
            for (final item in items) {
              ...
              Logger.log(
                  '프로필 체크: id=$profileId, userId=$profileUserId, name=${profile['name']}',
                  name: 'AWSProfileService');

              // userId 또는 id가 일치하는 경우
              if (profileUserId == userId || profileId == userId) {
                Logger.log('✅ 매칭되는 프로필 발견: ${profile['name']}',
                    name: 'AWSProfileService');
            ...
            Logger.log(
                '🔍 사용 가능한 userId들: ${items.map((item) => (item as Map<String, dynamic>)['userId']).toList()}',
                name: 'AWSProfileService');
          } else {
            Logger.log('❌ 프로필 데이터가 비어있음', name: 'AWSProfileService');
          }
        } catch (parseError) {
          Logger.error('GraphQL 응답 파싱 오류: $parseError',
              name: 'AWSProfileService');
        }
      } else {
    ...
    for (final entry in dynamoData.entries) {
      ...
      if (value is Map<String, dynamic>) {
        ...
        if (value.containsKey('S')) {
          ...
          Logger.log('  → String: $key = "$stringValue"',
              name: 'AWSProfileService');
        } else if (value.containsKey('N')) {
          // Number 타입
          final numStr = value['N'] as String;
          final numValue =
              numStr.contains('.') ? double.parse(numStr) : int.parse(numStr);
          ...
          Logger.log('  → Boolean: $key = $boolValue',
              name: 'AWSProfileService');
        } else if (value.containsKey('L')) {
          // List 타입
          final list = value['L'] as List;
          final convertedList =
              list.map((item) => _convertDynamoDBValue(item)).toList();
          converted[key] = convertedList;
          Logger.log('  → List: $key = $convertedList',
              name: 'AWSProfileService');
  ...
  void _analyzeDynamoDBResponse(dynamic responseData, String source) {
    ...
    Logger.log('   응답 타입: ${responseData.runtimeType}',
        name: 'AWSProfileService');

    if (responseData is Map) {
      Logger.log('   최상위 키들: ${responseData.keys.toList()}',
          name: 'AWSProfileService');
      ...
      responseData.forEach((key, value) {
        ...
        if (value is Map && value.isNotEmpty) {
          ...
          if (value.first is Map) {
            ...
            Logger.log('     첫 번째 아이템 키들: $firstItemKeys',
                name: 'AWSProfileService');
          }
        }
      });
    } else if (responseData is List) {
      Logger.log('   리스트 크기: ${responseData.length}',
          name: 'AWSProfileService');
      ...
      Logger.log('   문자열 길이: ${responseData.length}',
          name: 'AWSProfileService');
      Logger.log(
          '   문자열 시작: ${responseData.substring(0, math.min(100, responseData.length))}',
          name: 'AWSProfileService');
    }
  }
    ...
    try {
      ...
      if (response.data != null) {
        ...
        if (items != null && items.isNotEmpty) {
          ...
          Logger.log('📋 GraphQL 프로필 데이터: ${profileData.toString()}',
              name: 'AWSProfileService');
          ...
          Logger.log('🔄 GraphQL 변환된 데이터: ${convertedData.toString()}',
              name: 'AWSProfileService');
  ...
  /// 샘플 프로필 생성 (개발/테스트 목적)
  List<ProfileModel> _generateSampleProfiles(
      String currentUserId, String? targetGender) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      sampleProfiles.add(ProfileModel(
        ...
        education: '대학교 졸업',
        height: isTargetFemale
            ? 160 + math.Random().nextInt(10)
            : 170 + math.Random().nextInt(15),
    ...
    Logger.log('생성된 샘플 프로필 수: ${sampleProfiles.length}',
        name: 'AWSProfileService');
    for (int i = 0; i < sampleProfiles.length && i < 3; i++) {
      final profile = sampleProfiles[i];
      Logger.log(
          '샘플 프로필 ${i + 1}: ${profile.name} (${profile.gender}) - ${profile.age}세',
          name: 'AWSProfileService');
    }
