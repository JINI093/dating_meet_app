...
class _AdminNoticeScreenState extends ConsumerState<AdminNoticeScreen> {
  ...
  Widget build(BuildContext context) {
    final noticeState = ref.watch(adminNoticeProvider);
          ...
          children: [
            Expanded(
              ...
              child: TextField(
                ...
                onSubmitted: (value) {
                  ref.read(adminNoticeProvider.notifier).search(value);
                },
              ),
            ),
            const SizedBox(width: AdminTheme.spacingM),
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: ['전체', '남성회원', '여성회원', '전체회원', 'VIP회원'].map((targetType) {
                ...
                onChanged: (value) {
                  ...
                  ref.read(adminNoticeProvider.notifier).filterByTargetType(targetType);
                },
              ),
            ),
            const SizedBox(width: AdminTheme.spacingM),
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                onChanged: (value) {
                  ...
                  ref.read(adminNoticeProvider.notifier).filterByStatus(status);
                },
              ),
            ),
            const SizedBox(width: AdminTheme.spacingM),
            IconButton(
              onPressed: () {
                ref.read(adminNoticeProvider.notifier).refresh();
              },
    ...
    return notices.map((notice) {
      return DataRow(
        cells: [
          ...
          DataCell(_buildTargetTypeChip(notice.targetType)),
  ...
  Widget _buildTargetTypeChip(NoticeTargetType targetType) {
    Color color;
    String text;

    switch (targetType) {
      case NoticeTargetType.male:
        color = Colors.blue;
        text = '남성';
        break;
      case NoticeTargetType.female:
        color = Colors.pink;
        text = '여성';
        break;
        ...
        itemBuilder: (context, index) {
          ...
          return ExpandableNoticeCard(
            ...
            onStatusChange: (status) {
              ref.read(adminNoticeProvider.notifier).updateNoticeStatus(
                    notice.id,
                    status,
                  );
            },
          ...
          children: [
            IconButton(
              onPressed: state.currentPage > 1
                  ? () => ref.read(adminNoticeProvider.notifier).previousPage()
                  : null,
            ...
            IconButton(
              onPressed: state.currentPage < state.totalPages
                  ? () => ref.read(adminNoticeProvider.notifier).nextPage()
                  : null,
  ...
  void _showNoticeDialog({NoticeModel? notice}) {
    ...
    NoticeTargetType selectedTargetType = notice?.targetType ?? NoticeTargetType.all;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<NoticeTargetType>(
                        value: selectedTargetType,
                        decoration: const InputDecoration(
                          labelText: '대상',
                          border: OutlineInputBorder(),
                        ),
                        items: NoticeTargetType.values.map((targetType) {
                          return DropdownMenuItem(
                            value: targetType,
                            child: Text(targetType.displayName),
                          );
                        }).toList(),
                        onChanged: (value) {
  ...
  Future<void> _saveNotice(
    ...
    try {
      ...
      if (noticeId == null) {
        await ref.read(adminNoticeProvider.notifier).createNotice(dto);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        await ref.read(adminNoticeProvider.notifier).updateNotice(noticeId, dto);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteNotice(NoticeModel notice) async {
    ...
    if (confirmed == true) {
      try {
        await ref.read(adminNoticeProvider.notifier).deleteNotice(notice.id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
