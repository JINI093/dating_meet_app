...
/// 남성회원 공지사항 화면
class AdminMaleNoticeScreen extends ConsumerStatefulWidget {
  const AdminMaleNoticeScreen({super.key});

  @override
  ConsumerState<AdminMaleNoticeScreen> createState() => _AdminMaleNoticeScreenState();
}

class _AdminMaleNoticeScreenState extends ConsumerState<AdminMaleNoticeScreen> {
  ...
  Widget build(BuildContext context) {
    final noticeState = ref.watch(adminMaleNoticeProvider);
          ...
          children: [
            Expanded(
              ...
              child: TextField(
                ...
                onSubmitted: (value) {
                  ref.read(adminMaleNoticeProvider.notifier).search(value);
                },
              ),
            ),
            const SizedBox(width: AdminTheme.spacingM),
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                onChanged: (value) {
                  ...
                  ref.read(adminMaleNoticeProvider.notifier).filterByStatus(status);
                },
              ),
            ),
            const SizedBox(width: AdminTheme.spacingM),
            IconButton(
              onPressed: () {
                ref.read(adminMaleNoticeProvider.notifier).refresh();
              },
        ...
        itemBuilder: (context, index) {
          ...
          return ExpandableNoticeCard(
            ...
            onStatusChange: (status) {
              ref.read(adminMaleNoticeProvider.notifier).updateNoticeStatus(
                    notice.id,
                    status,
                  );
            },
          ...
          children: [
            IconButton(
              onPressed: state.currentPage > 1
                  ? () => ref.read(adminMaleNoticeProvider.notifier).previousPage()
                  : null,
            ...
            IconButton(
              onPressed: state.currentPage < state.totalPages
                  ? () => ref.read(adminMaleNoticeProvider.notifier).nextPage()
                  : null,
  ...
  Future<void> _saveNotice(
    ...
    try {
      ...
      if (noticeId == null) {
        await ref.read(adminMaleNoticeProvider.notifier).createNotice(dto);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        await ref.read(adminMaleNoticeProvider.notifier).updateNotice(noticeId, dto);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _deleteNotice(NoticeModel notice) async {
    ...
    if (confirmed == true) {
      try {
        await ref.read(adminMaleNoticeProvider.notifier).deleteNotice(notice.id);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
